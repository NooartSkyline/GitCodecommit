...
namespace Webservice_DohomeApplication
{
    ...
    public class Check_Position_Product_Contaminated : System.Web.Services.WebService
    {
                    ...
                    using (SqlTransaction tran = conn.BeginTransaction())
                    {
                        try
                        {
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                command.Parameters.AddWithValue("@DATECREATE", DateTime.Now.ToString());
        ...
        public string GetBinCode_From_TBTrans_Check_Position_Product_Contaminated(string BranchCode, string Site, string Sloc, string BinCode, string EmpCode)
        {
            ...
            string sDate = DateTime.Now.ToString("yyyy/dd/MM", new CultureInfo("en-US"));
            command.CommandText = "select DOCNO, DATECREATE, BRANCH, SITE, SLOC, BINCODE, EMPLOYEECODE from DBTRANS..TBTrans_Check_Position_Product_Contaminated"
                                ...
                                + " Group by DOCNO, DATECREATE, BRANCH, SITE, SLOC, BINCODE, EMPLOYEECODE";
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();
            SqlDataAdapter da = new SqlDataAdapter(command);
            da.Fill(dt);
        ...
        [WebMethod]
        public string GetBinCode_Detail_From_TBTrans_Check_Position_Product_Contaminated(string DOCNO, string BinCode)
        {
            dev.lib.Utilities util = new dev.lib.Utilities();
            SqlConnection conn = new SqlConnection();
            conn.ConnectionString = Connect;
