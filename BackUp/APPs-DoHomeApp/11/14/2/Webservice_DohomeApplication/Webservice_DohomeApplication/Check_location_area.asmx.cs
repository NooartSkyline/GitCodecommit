...
namespace Webservice_DohomeApplication
{
    ...
    public class Check_location_area : System.Web.Services.WebService
    {
                    ...
                    using (SqlTransaction tran = conn.BeginTransaction())
                    {
                        try
                        {
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                command.Parameters.AddWithValue("@DATECREATE", DateTime.Now.ToString("yyyy/dd/MM", new CultureInfo("en-US")));
        ...
        [WebMethod]
        public string GetBinCode_Detail_From_TBTrans_Check_Product_Location(string BinCode,string Docno)
        {
            dev.lib.Utilities util = new dev.lib.Utilities();
            SqlConnection conn = new SqlConnection();
            conn.ConnectionString = Connect;
